...
public class JdbcStudentDao implements StudentDao {
  ...
  public Student findByNo(int no) {
    try (Connection con = DriverManager.getConnection(
        ...
        Statement stmt = con.createStatement();
        ResultSet rs = stmt.executeQuery(
            "select student_id, name, tel, pst_no, bas_addr, det_addr, "
                + "work, gender, level, created_date "
  ...
  public void update(Student s) {
    try (Connection con = DriverManager.getConnection(
      ...
      String sql = String.format(
          "update app_student set name='%s', tel='%s', pst_no='%s', bas_addr='%s', det_addr='%s', "
              + "work=%b, gender='%s', level=%d where student_id=%d",
              s.getName(), s.getTel(), s.getPostNo(), s.getBasicAddress(), s.getDetailAddress(),
              s.isWorking(), s.getGender(), s.getLevel(), s.getNo());
      stmt.executeUpdate(sql);
  ...
  public boolean delete(Student s) {
    try (Connection con = DriverManager.getConnection(
      ...
      String sql = String.format("delete from app_student where student_id=%d",
          s.getNo());
